...
def cargar_datos():
    ...
    try:
        ...
        for registro in registros:
            ...
            # Evita error si el campo se guardó mal
            cve = registro.get("cveGru") or registro.get("cveGrupo","")
            nom = registro.get("nomGru","")
...
def eliminar_datos():
    ...
    respuesta = messagebox.askyesno(
        "Confirmar",
        f"¿Eliminar el grupo {cve_eliminar}?"
    )
...
lbl_cveGru = tk.Label(frame_form,text="Clave (cveGru):")
...
lbl_nomGru = tk.Label(frame_form,text="Nombre (nomGru):")
...
btn_Guardar = tk.Button(frame_botones,text="Guardar",command=guardar_datos,bg="#4CAF50",width=12)
btn_Guardar.grid(row=0,column=0,padx=5)

btn_Actualizar = tk.Button(frame_botones,text="Actualizar",command=actualizar_datos,bg="#2196F3",width=12,state='disabled')
btn_Actualizar.grid(row=0,column=1,padx=5)

btn_Eliminar = tk.Button(frame_botones,text="Eliminar",command=eliminar_datos,bg="#f44336",width=12)
btn_Eliminar.grid(row=0,column=2,padx=5)

btn_Limpiar = tk.Button(frame_botones,text="Limpiar",command=limpiar_campos,bg="#9E9E9E",width=12)
